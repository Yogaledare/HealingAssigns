...
public static class AssignmentEndpoints
{
    public static void MapAssignmentEndpoints(this WebApplication app)
    {
        ...
        {
            var assignment = await svc.Create(encounterId, req);
            return assignment is not null
                ? Results.Created($"/assignments/{assignment.Id}", assignment)
                : Results.BadRequest();
        }).RequireAuthorization();

        app.MapPut("/assignments/{id}", async (int id, UpdateAssignmentRequest req, AssignmentService svc) =>
            await svc.Update(id, req) is { } a ? Results.Ok(a) : Results.NotFound())
            .RequireAuthorization();
